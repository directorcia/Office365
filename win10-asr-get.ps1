...
write-host -foregroundcolor $processmessagecolor "Test for elevated privileges`n"
if (-not(amiadmin)) {
...
$displaycolor = @{
    0 = $errormessagecolor
    1 = $processmessagecolor
    2 = $auditmessagecolor
    6 = $warningmessagecolor
}

## https://docs.microsoft.com/en-us/powershell/module/defender/?view=win10-ps
try {
    $results = Get-MpPreference -ErrorAction Stop
}
catch {
    write-host -foregroundcolor $errormessagecolor "*** ERROR *** - Unable to read Defender preferences: $($_.Exception.Message)"
    exit 1
}

$configuredRules = @{}
for ($i = 0; $i -lt $results.AttackSurfaceReductionRules_ids.Count; $i++) {
    $configuredRules[$results.AttackSurfaceReductionRules_ids[$i]] = $results.AttackSurfaceReductionRules_actions[$i]
}
...
if (-not [string]::isnullorempty($results.AttackSurfaceReductionRules_ids)) {
    foreach ($rule in $asrrules) {
        if ($configuredRules.ContainsKey($rule.GUID)) {
            $enabled = $configuredRules[$rule.GUID]
            if ($enabledvalues.ContainsKey($enabled)) {
                write-host -foregroundcolor $displaycolor[$enabled] ($rule.Name + "=" + $enabledvalues[$enabled])
            }
            else {
                write-host -foregroundcolor $warningmessagecolor ($rule.Name + "= Unknown action value ($enabled)")
            }
        }
        else {
            write-host -foregroundcolor $errormessagecolor ($rule.Name + "= Not found")
        }
    }
}
else {
    write-host -foregroundcolor $errormessagecolor ($asrrules.count.ToString() + " ASR rules empty")
}
