...
function Test-ModuleAvailable {
    ...
    if (Get-Module -ListAvailable -Name $ModuleName) {
        write-host -ForegroundColor $processmessagecolor $ModuleName,"module found"
        return $true
    }

    write-host -ForegroundColor yellow -BackgroundColor $errormessagecolor "`n[002] - Online PowerShell module",$ModuleName,"not installed. Install-Module $ModuleName -Scope CurrentUser`n"
    return $false
}

function Get-ScriptPath {
    param([string]$Name)
    return (Join-Path -Path $PSScriptRoot -ChildPath $Name)
}
...
foreach ($entry in $scripts) {
    $scriptLookup[$entry.Service] = $entry
}
...
if (-not $noprompt) {
    try {
        ...
        write-host -ForegroundColor yellow -backgroundcolor $errormessagecolor "`n[001] - Error getting options via Out-GridView. Falling back to console selection.`n"
        $sorted = $scripts | Sort-Object Service
        for ($i = 0; $i -lt $sorted.Count; $i++) {
            write-host ("[{0}] {1}" -f $i, $sorted[$i].Service)
        }

        $manualSelection = Read-Host "Enter comma separated numbers to select services (blank to cancel)"
        ...
        $indices = $manualSelection -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ -match '^\d+$' }
        ...
        if (-not $results) {
            write-host -ForegroundColor yellow -backgroundcolor $errormessagecolor "No valid selections made."
            if ($debug) {
                Stop-Transcript | Out-Null
            }
            exit 1
...
foreach ($result in $results) {
    $script = $scriptLookup[$result.Service]

    if (-not $script) {
        write-host -ForegroundColor yellow -backgroundcolor $errormessagecolor "`n[004] - No script mapping found for service`n"
        continue
    }

    $run = Get-ScriptPath -Name $script.Name

    if (-not (Test-ModuleAvailable -ModuleName $script.Module)) {
        if ($debug) {
            Stop-Transcript | Out-Null
        }
        exit 2
